 ...
 * 次のFlow提案プロンプトモジュール
 */
export const nextFlowPromptModule: PromptModule<NextFlowContext> = merge(updateStatePromptModule, {
  ...
  inputs: [
    (ctx: NextFlowContext) =>
      `現在時刻: ${ctx.contextAnalysis.timeContext.currentTime.toISOString()}`,
    (ctx: NextFlowContext) => `タイムゾーン: ${ctx.contextAnalysis.timeContext.timezone}`,
    (ctx: NextFlowContext) =>
      `勤務時間内: ${ctx.contextAnalysis.timeContext.isWorkingHours ? 'はい' : 'いいえ'}`,
    (ctx: NextFlowContext) =>
      `ユーザーエネルギー: ${ctx.contextAnalysis.userContext.currentEnergy}`,
    (ctx: NextFlowContext) => `利用可能時間: ${ctx.contextAnalysis.userContext.availableTime}分`,
    '',
    (ctx: NextFlowContext) =>
      `アクティブFlow数: ${ctx.contextAnalysis.flowContext.activeFlows.length}`,
    (ctx: NextFlowContext) =>
      `締切が近いFlow数: ${ctx.contextAnalysis.flowContext.upcomingDeadlines.length}`,
    (ctx: NextFlowContext) =>
