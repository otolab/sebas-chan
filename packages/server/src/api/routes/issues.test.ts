...
describe('Issues API Routes', () => {
  ...
  describe('GET /issues', () => {
    ...
    it('should search issues with query parameter', async () => {
      ...
      const response = await request(app).get('/issues?q=login').expect(200);
  ...
  describe('GET /issues/:id', () => {
    it('should return issue by id', async () => {
      const response = await request(app).get('/issues/issue-123').expect(200);
    ...
    it('should handle errors', async () => {
      ...
      const response = await request(app).get('/issues/non-existent').expect(500);
  ...
  describe('POST /issues', () => {
    it('should create new issue', async () => {
      ...
      const response = await request(app).post('/issues').send(newIssue).expect(201);
    ...
    it('should handle optional fields', async () => {
      ...
      const response = await request(app).post('/issues').send(minimalIssue).expect(201);
  ...
  describe('PATCH /issues/:id', () => {
    it('should update issue', async () => {
      ...
      const response = await request(app).patch('/issues/issue-123').send(update).expect(200);
