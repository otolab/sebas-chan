...
export const analysisModule: PromptModule<AnalysisContext> = {
  ...
  instructions: [  // 標準セクション名（instructions大セクションに分類）
    '以下の観点で判定してください：',
    '- リクエストの種類',
    '- 必要なアクション',
    '- 発行すべきイベント'
...
**注意**: 同じセクションがある場合、内容が**結合**されます（上書きではありません）。
...
- [ ] 標準セクションを正しい大セクションに分類しているか
  - instructions大セクション: objective, terms, methodology, instructions, guidelines
  - data大セクション: state, inputs, materials, chunks
  - output大セクション: output
...
- [ ] ユーザーデータをdata大セクションの標準セクション（inputs/materials/chunks）に隔離しているか
- [ ] output.schemaで構造化出力を定義しているか
