...
#### 何を出すか（イベントとして）
- 問題報告が含まれる場合 → `ISSUE_REPORTED`イベント
...
3. **異常検出**: エラー、警告、アノマリーの識別
4. **重要度評価**: 即座の対応が必要かを判定
5. **メタデータ抽出**: 後の検索・分析のための情報抽出

#### 何を出すか（イベントとして）
- エラー検出時 → `ERROR_DETECTED`イベント
- 異常パターン検出時 → `ANOMALY_DETECTED`イベント
...
#### 本質的な役割
報告された問題や検出された異常が、システム全体にどのような影響を与えるかを分析し、対応の優先度を決定する。

#### 何を得るか
- 問題の詳細（エラーメッセージ、スタックトレース、症状）
- 発生コンテキスト（時刻、環境、関連システム）
- 過去の類似問題

#### 何をするか
1. **影響範囲の特定**: どのコンポーネント/ユーザーが影響を受けるか
2. **ビジネスインパクト評価**: 業務への影響度を推定
3. **技術的複雑度分析**: 解決の難易度を評価
4. **関連問題の発見**: 同じ根本原因を持つ可能性のある問題を特定
5. **対応優先度の決定**: 緊急度と重要度から優先度を算出

#### 何を出すか（イベントとして）
- 高優先度の問題 → `CRITICAL_ISSUE_IDENTIFIED`イベント
...
#### 本質的な役割
経験や情報から再利用可能な知識を抽出し、将来の問題解決に活用できる形で構造化・保存する。
